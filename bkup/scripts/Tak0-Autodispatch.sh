...
# ─────────────────────────────────────────────────────────────────────────────
#   $1            → target workspace
#   Next args     → optional capture rules (class:^regex$ syntax)
#   "--"          → argument separator
...
cleanup() {
  ...
  hyprctl keyword "windowrule match:initial_class .*, workspace unset" >>"$LOGFILE" 2>&1 || true
  for RULE in "${CAPTURE_RULES[@]}"; do
    echo "Cleanup: removing temporary capture rule: $RULE" >>"$LOGFILE"
    hyprctl keyword "windowrule ${RULE/class:/match:initial_class }, workspace unset" >>"$LOGFILE" 2>&1 || true
  done
}
...
hyprctl keyword \
  "windowrule match:initial_class .*, workspace $TARGET_WS silent" \
  >>"$LOGFILE" 2>&1 || true
...
for RULE in "${CAPTURE_RULES[@]}"; do
  ...
  hyprctl keyword \
    "windowrule ${RULE/class:/match:initial_class }, workspace $TARGET_WS silent" \
    >>"$LOGFILE" 2>&1 || true
done
